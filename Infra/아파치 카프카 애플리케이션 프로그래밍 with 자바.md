...
- 카프카 스트림즈는 카프카에서 공식적으로 지원하는 라이브러리
- 스트림 데이터 처리를 위해 아파치 스파크, 아파치 플링크, 아파치 스톰, 플루언트디와 같은 오픈소스 존재
- 프로듀서와 컨슈머를 조합해서 사용하지 않고 스트림즈를 사용
- 스트림즈 애플리케이션은 내부적으로 스레드를 1개 이상 생성. 스레드는 1개 이상의 태스크를 가짐
- 태스크는 실행하면서 생기는 데이터 처리 최소 단위
- 트리 형태의 토폴로지 사용
- 토폴로지는 이루는 노드를 하나의 프로세서라 함
- 노드와 노드를 이은 선을 스트림이라 함
- 프로세서의 구분
